...
import { Banknote, FileText, Calendar, XCircle, CheckCircle } from "lucide-react";
...
export function IncomeFormDialog({ open, onOpenChange, onSuccess }: IncomeFormDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      accountingService.getIncomeCategories().then(setCategories);
      form.reset({
        amount: undefined as unknown as number,
        description: "",
        date: new Date().toISOString().split("T")[0],
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    setLoading(true);
    try {
      await accountingService.createIncome(values);
      toast.success(t("common.success"), {
        description: t("financial.incomeCreateSuccess"),
      });
      onSuccess();
      onOpenChange(false);
    } catch {
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="sm:max-w-lg p-0 flex flex-col h-full">
        <SheetHeader className="px-6 pt-6 shrink-0">
          ...
          <SheetDescription>{t("financial.addIncomeDescription")}</SheetDescription>
        </SheetHeader>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>{t("financial.incomeCategoryDescription")}</FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              />
